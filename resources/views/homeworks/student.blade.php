...
<div class="col-lg-6"><div class="card stat-card h-100"><div class="card-body">
    ...
    @if($h->materials->count())
    <div class="border rounded p-3 mb-3">
        <div class="fw-semibold mb-2">Материалы преподавателя</div>
        @foreach($h->materials as $material)
            @if($material->type==='file')
                <div class="d-flex align-items-center gap-2 mb-2">
                    <span class="badge text-bg-primary">Файл</span>
                    <span class="flex-grow-1">{{ $material->original_name }}</span>
                    @if($material->isPreviewable())<a target="_blank" class="btn btn-sm btn-outline-primary" href="{{ route('homeworks.materials.view',$material) }}">Просмотр</a>@endif
                    <a class="btn btn-sm btn-outline-secondary" href="{{ route('homeworks.materials.download',$material) }}">Скачать</a>
                </div>
            @elseif($material->type==='video')
                <div class="mb-2"><span class="badge text-bg-danger me-2">Видео</span><a href="{{ $material->url }}" target="_blank" rel="noopener">{{ $material->url }}</a></div>
            @else
                <div class="mb-2"><span class="badge text-bg-info me-2">Ссылка</span><a href="{{ $material->url }}" target="_blank" rel="noopener">{{ $material->url }}</a></div>
            @endif
        @endforeach
    </div>
    @endif
