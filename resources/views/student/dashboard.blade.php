...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-white d-flex justify-content-between align-items-center"><strong>Последние уроки и лекции</strong><a class="small" href="{{ route('lessons.content.index') }}">Все материалы</a></div>
    <div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Дата</th><th>Предмет</th><th>Тип</th><th>Тема</th><th>Материалы</th><th></th></tr></thead><tbody>
    @forelse($recentLessons as $lesson)
    <tr><td>{{ $lesson->lesson_date?->format('d.m.Y') }}</td><td>{{ $lesson->subject?->name }}</td><td>{{ $lesson->lesson_kind ?: ($lesson->workType?->name ?? 'Занятие') }}</td><td><strong>{{ $lesson->topic ?: 'Без темы' }}</strong></td><td>{{ $lesson->materials->count() }}</td><td><a class="btn btn-sm btn-outline-primary" href="{{ route('lessons.content.show',$lesson) }}">Открыть</a></td></tr>
    @empty<tr><td colspan="6" class="text-center text-muted py-4">Материалов уроков пока нет.</td></tr>@endforelse
    </tbody></table></div>
</div>
