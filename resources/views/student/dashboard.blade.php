                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    @forelse($subjectStats as $stat)
                        <tr>
                            ...
                            <td>
                                @if($stat['final_grade'])<span class="badge text-bg-success fs-6">{{ $stat['final_grade'] }}</span>@else<span class="text-muted">не выставлена</span>@endif
                                @if($stat['final_comment'])<div class="small text-muted mt-1">{{ $stat['final_comment'] }}</div>@endif
                            </td>
        ...
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="card-body p-0">
                @forelse($debts as $debt)
                    <div class="p-3 border-bottom"><div class="d-flex justify-content-between gap-2"><strong>{{ $debt['title'] }}</strong><span class="badge text-bg-danger">2</span></div><div class="small text-muted">{{ $debt['subject'] }} · {{ $debt['type'] }} @if($debt['date'])· {{ $debt['date'] }}@endif</div></div>
                @empty<div class="p-4 text-center text-success">Задолженностей нет.</div>@endforelse
            </div>
        </div>
    </div>
</div>
...
<div class="row g-4 mb-4">
    <div class="col-xl-6"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white d-flex justify-content-between"><strong>Ближайшие домашние задания</strong><a class="small" href="{{ route('homeworks.index') }}">Все</a></div><div class="card-body p-0">
        @forelse($upcomingHomeworks as $homework)
            @php($submission = $homework->my_submission)
            <div class="p-3 border-bottom"><div class="d-flex justify-content-between gap-3"><div><strong>{{ $homework->title }}</strong><div class="small text-muted">{{ $homework->subject?->name }} @if($homework->workType)· {{ $homework->workType->name }} ×{{ $homework->grade_weight }}@endif</div></div><div class="text-end small">@if($homework->due_at)<div>до {{ $homework->due_at->format('d.m.Y H:i') }}</div>@endif @if($submission)<span class="badge {{ $submission->grade ? 'text-bg-success' : 'text-bg-info' }}">{{ $submission->grade ? 'Оценка '.$submission->grade : 'Сдано' }}</span>@else<span class="badge text-bg-warning">Не сдано</span>@endif</div></div></div>
        @empty<div class="p-4 text-center text-muted">Ближайших заданий нет.</div>@endforelse
    </div></div></div>
    <div class="col-xl-6"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white"><strong>История домашних работ</strong></div><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Работа</th><th>Тип/вес</th><th>Статус</th><th>Оценка</th></tr></thead><tbody>
        @forelse($submissions as $submission)<tr><td><strong>{{ $submission->homework?->title }}</strong><div class="small text-muted">{{ $submission->homework?->subject?->name }}</div></td><td>{{ $submission->homework?->workType?->name ?? 'ДЗ' }} ×{{ $submission->homework?->grade_weight ?? 1 }}</td><td>@if($submission->status==='graded')<span class="badge text-bg-success">Проверено</span>@elseif($submission->status==='returned')<span class="badge text-bg-warning">Доработка</span>@else<span class="badge text-bg-info">На проверке</span>@endif</td><td class="fw-bold">{{ $submission->grade ?? '—' }}</td></tr>
        @empty<tr><td colspan="4" class="text-center text-muted py-4">Сданных работ пока нет.</td></tr>@endforelse
    </tbody></table></div></div></div>
</div>

<div class="card border-0 shadow-sm"><div class="card-header bg-white"><strong>Последние отметки в журнале</strong></div><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Дата</th><th>Дисциплина</th><th>Тип</th><th>Тема</th><th>Посещение</th><th>Оценка</th></tr></thead><tbody>
@forelse($recentRecords as $record)<tr><td>{{ $record->lesson?->lesson_date?->format('d.m.Y') }}</td><td>{{ $record->lesson?->subject?->name }}</td><td>{{ $record->lesson?->workType?->name ?? 'Занятие' }} @if($record->grade)<span class="text-muted small">×{{ $record->lesson?->grade_weight ?? 1 }}</span>@endif</td><td>{{ $record->lesson?->topic ?: '—' }}</td><td>@if($record->attendance==='present')<span class="badge text-bg-success">Присутствовал</span>@elseif($record->attendance==='late')<span class="badge text-bg-warning">Опоздал</span>@elseif($record->attendance==='excused')<span class="badge text-bg-info">Уважительно</span>@else<span class="badge text-bg-danger">Прогул</span>@endif</td><td class="fw-bold">{{ $record->grade ?? '—' }}</td></tr>
@empty<tr><td colspan="6" class="text-center text-muted py-4">Записей пока нет.</td></tr>@endforelse
</tbody></table></div></div>
@endsection
